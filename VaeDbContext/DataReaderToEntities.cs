...
{
    public static class DataReaderToEntities
    {
        /// <summary>
        /// DataReader转泛型
        /// </summary>
        ...
        public static IList<T> ReaderToList<T>(this IDataReader objReader)
        {
            using (objReader)
            {
                ...
                var properties = modelType.GetProperties().ToDictionary(p => p, p => p.GetCustomAttribute<ColumnAttribute>());
                //遍历DataReader对象
                while (objReader.Read())
                {
                    ...
                    for (int i = 0; i < objReader.FieldCount; i++)
                    {
                        ...
                        if (!IsNullOrDBNull(objReader[i]))
                        {
                            ...
                            PropertyInfo pi = properties.FirstOrDefault(p => p.Value.Name == objReader.GetName(i)).Key;
                            if (pi != null)
                            {
                                //绑定实体对象中同名的字段  
                                if (pi.PropertyType.IsEnum)
                                    pi.SetValue(model, Enum.Parse(pi.PropertyType, Enum.GetName(pi.PropertyType, objReader[i])), null);
                                else
